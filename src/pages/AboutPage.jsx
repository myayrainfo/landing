...
const orbitSlots = [
  { x: 0, y: 0, scale: 1, opacity: 1, z: 50, blur: 0, rotate: 0 },
  { x: 152, y: -104, scale: 0.74, opacity: 0.62, z: 30, blur: 0.4, rotate: 10 },
  { x: 134, y: 126, scale: 0.66, opacity: 0.5, z: 20, blur: 0.8, rotate: 5 },
  { x: -134, y: 126, scale: 0.66, opacity: 0.5, z: 20, blur: 0.8, rotate: -5 },
  { x: -152, y: -104, scale: 0.74, opacity: 0.62, z: 30, blur: 0.4, rotate: -10 },
];
...
export default function AboutPage() {
  ...
  const markPhotoMissing = (name) => {
    setMissingPhotos((current) => (current[name] ? current : { ...current, [name]: true }));
  };
                  ...
                  >
                    <div
                      ...
                      <div className={`rounded-[1.5rem] bg-gradient-to-br ${member.accent} p-3`}>
                        {missingPhotos[member.name] ? (
                          <div className="flex h-36 w-full items-center justify-center rounded-[1.25rem] bg-white/60 text-center sm:h-40">
                            <div>
                              <p className="font-headline text-2xl font-bold text-[#131b2e]">
                                {member.name
                                  .split(" ")
                                  .map((part) => part[0])
                                  .join("")
                              ...
                              <p className="mt-2 text-xs uppercase tracking-[0.18em] text-[#6d7683]">
                                Photo
                              </p>
                            </div>
                          </div>
