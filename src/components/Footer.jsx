    ...
    <footer className="w-full border-t border-slate-200 bg-white pt-16">
          ...
          <div key={title} className="flex flex-col gap-4">
            ...
            {links.map((link) => (
              <RouterLink
                key={link.label}
                to={link.to}
                className="text-sm text-slate-500 transition-colors hover:text-[#004a99]"
              >
                {link.label}
              </RouterLink>
            ))}
      ...
      <div className="border-t border-slate-100">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-5 py-8 text-xs text-slate-400 sm:px-8 md:flex-row">
          <p>&copy; 2024 AYRA. All rights reserved. Built with precision.</p>
          <div className="flex gap-6">
            <RouterLink to="/contact" className="hover:text-[#00346f]">
              Privacy Policy
            </RouterLink>
            <RouterLink to="/contact" className="hover:text-[#00346f]">
              Terms of Service
